...
export default function AuthForm({
  ...
  const onLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // 1. Cek apakah ada 'next' URL dari middleware?
      const nextUrl = searchParams.get("next");

      if (nextUrl) {
        router.push(nextUrl); // Balik ke halaman sebelumnya (misal Admin)
      } else {
        router.push("/"); // DEFAULT: Ke Home
      }

      router.refresh();
      if (onSuccess) onSuccess(); // Tutup dialog jika dalam mode popup
      setLoading(false);
    }
  };
  ...
  const onRegister = async (e: React.FormEvent) => {
    ...
    if (data.session) {
      ...
      // Register Sukses -> Langsung Redirect ke Home
      router.push("/");
      router.refresh();
      if (onSuccess) onSuccess();
    } else {
      setIsRegisterSuccess(true); // Tampilkan pesan cek email jika confirm required
    }
    setLoading(false);
  };
        ...
        <div className="relative z-10 px-6">
          ...
          <p className="text-blue-100 text-sm max-w-xs mx-auto leading-relaxed">
            {activeTab === "login"
              ...
              : "Bergabung sekarang untuk akses penuh."}
          </p>
        </div>
      </div>
      ...
      <div className="p-6 md:p-8 bg-white relative">
        {isRegisterSuccess ? (
          ...
          <>
            ...
            >
              <GoogleIcon grayscale /> <span>Google (Segera Hadir)</span>
            </Button>
            ...
            >
              <TabsList className="grid w-full grid-cols-2 p-1 bg-slate-100 rounded-xl mb-6">
                <TabsTrigger
                  value="login"
                  className="rounded-lg text-sm font-medium py-2.5"
                >
                  Masuk
                </TabsTrigger>
                <TabsTrigger
              ...
              >
                <form onSubmit={onRegister} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <Lock
                        className={`absolute left-3.5 top-3.5 h-4 w-4 ${isPasswordMismatch ? "text-red-400" : "text-slate-400"}`}
                      />
                      <Input
                        ...
                        required
                        className={`h-11 pl-10 pr-10 rounded-lg bg-slate-50 border-slate-200 focus:bg-white focus:ring-2 focus:border-blue-500 ${isPasswordMismatch ? "border-red-300 focus:ring-red-100 focus:border-red-500" : "focus:ring-blue-100"}`}
                      />
                      <button
                        type="button"
                  ...
                  >
                    {loading ? (
                      ...
                      <span className="flex items-center">
                        Buat Akun Gratis <ArrowRight className="w-4 h-4 ml-2" />
                      </span>
                    )}
                  </Button>
